...
- (void)setProps:(NSDictionary *_Nonnull)newProps {
    ...
    if (rootView) {
        ...
        mEmitter = [[ModuleWithEmitter alloc] init];
        mEmitter.parentView = self;
        [mEmitter setBridge:bridge];
    }
}
...
- (void)testMethodToCheckFramework{
    NSLog(@"Hello Jio-meet");
}
//- (void)sendCommandToJitsi{
//    if (mEmitter) {
//        [mEmitter muteAudio1];
//    }
//}
...
-(void)didUpdateParticipantsData:(NSDictionary*)participants{
    if ([self.delegate respondsToSelector:@selector(didChangeParticipantsData:)]) {
        [self.delegate didChangeParticipantsData:participants];
    }
}
@end
