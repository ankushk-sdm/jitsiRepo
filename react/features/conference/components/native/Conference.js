...
import { NativeModules, SafeAreaView, StatusBar, Alert } from 'react-native';
...
import { CHAT_VIEW_MODAL_ID } from '../../../chat/constants';
...
// const iOSModuleWithEmitter = new NativeEventEmitter(ModuleWithEmitter);
...
type Props = AbstractProps & {
    ...
    //Srinivas
    _latestJoinedUser: string,

    _latestLeftUser: string,
...
class Conference extends AbstractConference<Props, *> {
    ...
    // doChnageAudioMuteState(audioMuted) {
    //     this.props.dispatch(setAudioMuted(audioMuted, /* ensureTrack */ true));
    // }
    // doChnageVideoMuteState(videoMuted) {
    //     this.props.dispatch(
    //         setVideoMuted(
    //             videoMuted,
    //             MEDIA_TYPE.VIDEO,
    ...
    // }
/*
    executeIOSCommands(dataObj){
        // Alert.alert(JSON.stringify(dataObj))
        const commandCode = dataObj["commandCode"];
        const commandData = dataObj["commandData"];
        console.log(`Command Code ${commandCode} --- ${commandData}`);
        switch (commandCode) {
            case commands.COMMAND_MUTE_AUDIO:
                this.doChnageAudioMuteState(true);
                break;
            case commands.COMMAND_UNMUTE_AUDIO:
                this.doChnageAudioMuteState(false);
                break;
            case commands.COMMAND_MUTE_VIDEO:
                this.doChnageVideoMuteState(true);
                break;
            case commands.COMMAND_UNMUTE_VIDEO:
                this.doChnageVideoMuteState(false);
                break;
            case commands.COMMAND_SWITCH_CAMERA:
                // this.props.dispatch(toggleCameraFacingMode());
                break;
            case commands.COMMAND_SEND_CHAT_MSG:
                // this.props.dispatch(sendMessage(commandData));
                break;
            case commands.COMMAND_GET_CHAT_MESSAGES:
                // var pageNumber = commandData;
                // this._doProcessGetMessagesReq(pageNumber);
                break;
            case commands.COMMAND_SWITCH_SHARESCREEN:
                // isShareScreenEnable = true;
                // this.props.dispatch(toggleTileView()); //changeModeToTileView
                break;
            case commands.COMMAND_SET_TILEMODE:
                // isShareScreenEnable = false;
                // this.props.dispatch(toggleTileView()); //changeModeToTileView
                // this.props.dispatch(changeModeToTileView()); //changeModeToTileView
                break;
            case commands.COMMAND_SET_CHATMODE:
                this.props.dispatch(setActiveModalId(CHAT_VIEW_MODAL_ID));
                break;
        }
    }
    */
    componentDidMount() {
        // iOSModuleWithEmitter.addListener(ModuleWithEmitter.MuteAudio123, e => {this.executeIOSCommands(e)});
...
function _mapStateToProps(state) {
    ...
    return {
        ...
        _latestJoinedUser: state['features/base/conference'].latestJoinedUser,
        _latestLeftUser: state['features/base/conference'].latestLeftUser,
        _trackStatus: state['features/base/conference'].trackStatus
    };
}
